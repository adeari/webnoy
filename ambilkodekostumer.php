...
//Show Method @2-7C2E8FDD
    function Show()
    {
		...
		$this->DataSource->Prepare("kd_kos like 'k%'");
		$this->DataSource->Open();
		if ($this->DataSource->next_record()) {
                    $this->DataSource->SetValues();
			$kangka=substr($this->DataSource->nota->GetValue(),1,4);
			$angka=intval($kangka)+1;
			$kangka="".$angka;
			do {
				$kangka="0".$kangka;
			} while (strlen($kangka)<4);
			$this->hasil->SetValue("k".$kangka);
		}
		else 		
			$this->hasil->SetValue("k0001");
...
class ceknotajualDataSource extends clsDBMyCon {
	...
	function Open()
    {
        ...
        $this->SQL = "SELECT max(kd_kos) as nota   FROM  tb_kostumer  {SQL_Where} {SQL_OrderBy}";
