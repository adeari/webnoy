...
//Show Method @2-7C2E8FDD
    function Show()
    {
		...
		$this->DataSource->Prepare("kd_sup like 's%'");
		$this->DataSource->Open();
		if ($this->DataSource->next_record()) {
                    $this->DataSource->SetValues();
			$kangka=substr($this->DataSource->nota->GetValue(),2,4);
			$angka=intval($kangka)+1;
			$kangka="".$angka;
			do {
				$kangka="0".$kangka;
			} while (strlen($kangka)<4);
			$this->hasil->SetValue("s".$kangka);
		}
		else 		
			$this->hasil->SetValue("s0001");
...
class ceknotajualDataSource extends clsDBMyCon {
	...
	function Open()
    {
        ...
        $this->SQL = "SELECT max(kd_sup) as nota   FROM  tb_suplier  {SQL_Where} {SQL_OrderBy}";
