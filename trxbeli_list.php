...
define("FileName", "trxbeli_list.php");
...
class clsRecordm_trxbeliSearch { //m_trxbeliSearch Class @2-7972187D
...
//Class_Initialize Event @2-5CCA453A
    function clsRecordm_trxbeliSearch($RelativePath, & $Parent)
    {
        ...
        $this->ErrorBlock = "Record m_trxbeliSearch/Error";
        $this->ReadAllowed = true;
        if($this->Visible)
        {
            $this->ComponentName = "m_trxbeliSearch";
...
//Operation Method @2-37A5AC5E
    function Operation()
    {
        ...
        $Redirect = "trxbeli_list.php";
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "trxbeli_list.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
} //End m_trxbeliSearch Class @2-FCB6E20C

class clsGridm_trxbeli { //m_trxbeli class @9-8624B687
...
//Class_Initialize Event @9-7CEF8FA3
    function clsGridm_trxbeli($RelativePath, & $Parent)
    {
        global $FileName;
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->ComponentName = "m_trxbeli";
        $this->Visible = True;
        $this->Parent = & $Parent;
        $this->RelativePath = $RelativePath;
        $this->Errors = new clsErrors();
        $this->ErrorBlock = "Grid m_trxbeli";
        $this->Attributes = new clsAttributes($this->ComponentName . ":");
        ...
        $this->SorterName = CCGetParam("m_trxbeliOrder", "");
        $this->SorterDirection = CCGetParam("m_trxbeliDir", "");

        $this->nota = & new clsControl(ccsLink, "nota", "nota", ccsText, "", CCGetRequestParam("nota", ccsGet, NULL), $this);
        $this->nota->Page = "trxbeli_isi.php";
		$this->kd_sup = & new clsControl(ccsLabel, "kd_sup", "kd_sup", ccsText, "", CCGetRequestParam("kd_sup", ccsGet, NULL), $this);
        ...
        $this->m_trxbeli_Insert = & new clsControl(ccsLink, "m_trxbeli_Insert", "m_trxbeli_Insert", ccsText, "", CCGetRequestParam("m_trxbeli_Insert", ccsGet, NULL), $this);
        $this->m_trxbeli_Insert->Parameters = CCGetQueryString("QueryString", array("nota", "ccsForm"));
        $this->m_trxbeli_Insert->Page = "trxbeli_isi.php";
        $this->Sorter_nota = & new clsSorter($this->ComponentName, "Sorter_nota", $FileName, $this);
        $this->Sorter_nama_sup = & new clsSorter($this->ComponentName, "Sorter_nama_sup", $FileName, $this);
		$this->Sorter_kd_sup = & new clsSorter($this->ComponentName, "Sorter_kd_sup", $FileName, $this);
...
//Show Method @9-3EABC7D3
    function Show()
    {
        ...
        $this->m_trxbeli_Insert->Show();
        $this->Sorter_nota->Show();
        $this->Sorter_nama_sup->Show();
        $this->Sorter_kd_sup->Show();
...
} //End m_trxbeli Class @9-FCB6E20C

class clsm_trxbeliDataSource extends clsDBMyCon {  //m_trxbeliDataSource Class @9-9E707F87
...
//SetOrder Method @9-828F292A
    function SetOrder($SorterName, $SorterDirection)
    {
        ...
        $this->Order = CCGetOrder($this->Order, $SorterName, $SorterDirection, 
            ...
            "Sorter_nama_sup" => array("nama_sup", ""), 
            "Sorter_kd_sup" => array("kd_sup", ""),
...
//Prepare Method @9-71314EEE
    function Prepare()
    {
        ...
        $this->wp->Criterion[2] = $this->wp->Operation(opContains, "nama_sup", $this->wp->GetDBValue("2"), $this->ToSQL($this->wp->GetDBValue("2"), ccsText),false);
        $this->wp->Criterion[3] = $this->wp->Operation(opContains, "kd_sup", $this->wp->GetDBValue("3"), $this->ToSQL($this->wp->GetDBValue("3"), ccsText),false);
...
//Open Method @9-6109A04B
    function Open()
    {
        ...
        "FROM tb_trxbeli";
        $this->SQL = "select nota,kd_sup,tanggal,nama_sup from (SELECT a.nota,a.kd_sup,date_format(a.tanggal,'%d/%m/%Y') as tanggal,b.nama_sup,a.tanggal as tanggals from tb_trxbeli a join tb_suplier b on a.kd_sup=b.kd_sup) as aha {SQL_Where} {SQL_OrderBy}";
...
} //End m_trxbeliDataSource Class @9-FCB6E20C
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "trxbeli_list.html";
$BlockToParse = "main";
...
include_once("./trxbeli_list_events.php");
...
// Controls
$m_trxbeliSearch = & new clsRecordm_trxbeliSearch("", $MainPage);
$m_trxbeli = & new clsGridm_trxbeli("", $MainPage);
...
$header->Initialize();
$MainPage->m_trxbeliSearch = & $m_trxbeliSearch;
