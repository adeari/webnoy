...
//Variables @2-D6FF3E86
    ...
    function clsRecordceknotajual($RelativePath, & $Parent)
    {
	   ...
	   $this->diawali = & new clsControl(ccsLabel, "diawali", "diawali", ccsText, "", CCGetRequestParam("diawali", ccsPost, NULL), $hasil);	
...
//Show Method @2-7C2E8FDD
    function Show()
    {
		...
		if (strcmp($this->diawali->GetValue(true),"")==0)
			$this->DataSource->Prepare("nota = '".$this->kol->GetValue(true)."'");
		else
			$this->DataSource->Prepare("nota = '".$this->kol->GetValue(true)."' and nota <> '".$this->diawali->GetValue(true)."'");
		$this->DataSource->Open();
		if ($this->DataSource->next_record()) {
                    $this->DataSource->SetValues();
			$this->hasil->SetValue($this->DataSource->nota->GetValue());
		}
		else 
...
class ceknotajualDataSource extends clsDBMyCon {
	...
	function Open()
    {
        ...
        $this->SQL = "SELECT nota   FROM  tb_trxjual  {SQL_Where} {SQL_OrderBy}";
