...
//Connection Settings @0-FC99657E
...
//Initialize Common Variables @0-6F38D3F6
...
//MyCon Connection Class @-D56BDED4
class clsDBMyCon extends DB_Adapter
{
    function clsDBMyCon()
    {
        $this->Initialize();
    ...
    function Initialize()
    {
        ...
        $this->SetProvider($CCConnectionSettings["MyCon"]);
        parent::Initialize();
...
//End MyCon Connection Class



//IntranetDB Connection Class @-929F76C3
class clsDBIntranetDB extends DB_Adapter
{
    function clsDBIntranetDB()
    {
        $this->Initialize();
    }
    ...
    function OptimizeSQL($SQL)
    {
        ...
        $Page = $this->AbsolutePage ? (int) $this->AbsolutePage : 1;
        if (strcmp($this->RecordsCount, "CCS not counted")) 
            $SQL = str_replace(array("{SqlParam_endRecord}", "{SqlParam_numRecords}", "{SqlParam_Offset}"),
                array(($Page  * $PageSize), $PageSize, (($Page - 1) * $PageSize)), $SQL);
        else
            $SQL = str_replace(array("{SqlParam_endRecord}", "{SqlParam_numRecords}", "{SqlParam_Offset}"),
                array(($Page  * $PageSize + 1), ($PageSize + 1), (($Page - 1) * $PageSize)), $SQL);
        return $SQL;
    }
...
//End IntranetDB Connection Class
